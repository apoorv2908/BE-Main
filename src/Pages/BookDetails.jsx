...
const BookDetails = () => {
  ...
  useEffect(() => {
    fetch(
      `${config.apiBaseUrl}/fullmarks-user/navbar/fetchbookdetails.php?book_id=${decodedId}`
    )
    ...
    fetch(
      `${config.apiBaseUrl}/fullmarks-user/navbar/fetchchapters.php?book_id=${decodedId}`
    )
      .then((response) => response.json())
